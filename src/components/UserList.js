...
const UserList = () => {
  ...
  const handleEdit = (userId) => {
    ...
    navigate(`/editar-usuario/${userId}`, { state: { userData: selectedUser } });
  };
        ...
        {users.map((user) => (
          <div key={user.id}>
            {/*Mandamos el id en la ruta de editar usuario*/ }
             <Link to={`/edituser/${user.id}`} state={{ userData: user }}></Link>
            <input className="user-list" type="text" defaultValue={user.nombre} readOnly />   
            <button className="editButton" onClick={() => handleEdit(user.id)}>
